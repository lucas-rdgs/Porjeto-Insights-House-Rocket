...
def data_cleaning(data):
    ...
    # Creating a columns "condition_descritption" with text description for the properties' condition
...
def get_data_purchase(data):
    ...
    for i in range(len(data_purchase)):
        if (data_purchase.loc[i, 'price'] < data_purchase.loc[i, 'price_median']) & (data_purchase.loc[i, 'condition'] >= 4):
...
def region_overview(data, geofile):
    ...
    for name, row in df.iterrows():
        iframe = folium.IFrame = (f"Sold for ${row['price']:,.2f} on: {row['date']:%Y-%m-%d}.  Condition: {row['condition_description']}. Features: {row['sqft_living']} sqft, "
                                  f"{row['bedrooms']} bedrooms, {row['bathrooms']:.0f} bathrooms, Year Built: {row['yr_built']}")
    ...
    for name, row in data_purchase_full.iterrows():
        iframe = folium.Iframe = (
            f"Sold for ${row['price']:,.2f}. Purchased on: {row['date']:%Y-%m-%d}. Condition: {row['condition_description']}. "
