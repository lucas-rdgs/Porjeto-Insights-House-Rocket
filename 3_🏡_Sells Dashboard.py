...
def interactive_sell(df: pd.DataFrame):
    ...
    #                         type=["numericColumn", "numberColumnFilter", "customNumericFormat"],
    #                         valueGetter="data.price_median.toLocaleString('en-US', {style: 'currency', currency: 'USD', maximumFractionDigits:2})")
...
def region_overview(data_sell_full, geofile):
    ...
    for name, row in df1.iterrows():
        iframe = folium.IFrame = (
            f"Selling price ${row['sell_price']:,.2f}. Purchased for ${row['price']:,.2f} on: {row['date']:%Y-%m-%d}. Condition: {row['condition_description']}. \nFeatures: {row['sqft_living']} sqft, "
...
def gain_map(data_sell_full, geofile):
...
def attributes_distribution(data_sell_full):
    ...
    # fig = px.pie(df, names='waterfront')
    c2.plotly_chart(fig, use_container_width=True)
...
if __name__ == "__main__":
    ...
    gain_map(data_sell_full, geofile)
